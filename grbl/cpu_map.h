...
//定义步进方向输出引脚。注意：所有方向引脚必须位于同一端口上。
  #define DIRECTION_DDR     P4
  #define DIRECTION_PORT    P4M0
...
//定义步进驱动器启用/禁用输出引脚。
  #define STEPPERS_DISABLE_DDR    P2
  #define STEPPERS_DISABLE_PORT   P2M0
...
//注意：所有控制管脚必须位于同一端口上，而不是位于具有其他输入管脚的端口上（限位）。
  ...
  #define PROBE_MASK      (1<<PROBE_BIT)
    ...
    //定义也冷和雾化冷却液启用输出引脚。
    #define COOLANT_FLOOD_DDR   P3
    #define COOLANT_FLOOD_PORT  P3
    #define COOLANT_FLOOD_BIT   3//Uno模拟引脚3
    #define COOLANT_MIST_DDR   P3
    #define COOLANT_MIST_PORT  P3
    #define COOLANT_MIST_BIT   4//Uno模拟引脚4

//定义主轴启用和主轴方向输出引脚。
    #define SPINDLE_ENABLE_DDR    P2
    #define SPINDLE_ENABLE_PORT   P2
...
//如果启用，主轴方向引脚现在用作主轴启用，而PWM保持在D11上。
      ...
      #define SPINDLE_DIRECTION_DDR   P2
      #define SPINDLE_DIRECTION_PORT  P2
