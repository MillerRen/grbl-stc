...
单词，以及为值单词F、N、P、T和S设置的负值。*/
  ...
  while (line[char_counter] != 0) { //循环，直到行中不再有g代码字。
    ...
    switch(letter) {
      ...
      case 'G':
        ...
        if ( bit_istrue(command_words,bit_mask(word_bit)) ) { FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); }
        command_words |= bit_mask(word_bit);
        break;
      ...
      default:
        ...
        if (bit_istrue(value_words,bit_mask(word_bit))) { FAIL(STATUS_GCODE_WORD_REPEATED); } //[字重复]
        
        //检查字F、N、P、T和S的无效负值。
        //注意：这里进行负值检查只是为了提高代码效率。
        if ( bit_mask(word_bit) & (bit_mask(WORD_F)|bit_mask(WORD_N)|bit_mask(WORD_P)|bit_mask(WORD_T)|bit_mask(WORD_S)) ) {
          if (value < 0.0) { FAIL(STATUS_NEGATIVE_VALUE); } //[字值不能为负]
        }
        value_words |= bit_mask(word_bit); //指示指定参数的标志。
  ...
  */
  ...
  //检查有效的行号N值。
  if (bit_istrue(value_words,bit_mask(WORD_N))) {
    //行号值不能小于零（完成）或大于最大行号。
    if (gc_block.values.n > MAX_LINE_NUMBER) { FAIL(STATUS_GCODE_INVALID_LINE_NUMBER); } //[超过最大行号]
  }
  // bit_false(value_words,bit_mask(WORD_N)); // NOTE: Single-meaning value word. Set at end of error-checking.
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
    if (bit_isfalse(value_words,bit_mask(WORD_F))) { FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE); }
    ...
    if (gc_block.modal.feed_rate == FEED_RATE_MODE_INVERSE_TIME) { // = G93
      ...
      if (axis_command == AXIS_COMMAND_MOTION_MODE) {
        if ((gc_block.modal.motion != MOTION_MODE_NONE) && (gc_block.modal.motion != MOTION_MODE_SEEK)) {
          if (bit_isfalse(value_words,bit_mask(WORD_F))) { FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE); } //[F单词缺失]
        }
      }
      ...
      if (gc_state.modal.feed_rate == FEED_RATE_MODE_UNITS_PER_MIN) { // Last state is also G94
        if (bit_istrue(value_words,bit_mask(WORD_F))) {
          if (gc_block.modal.units == UNITS_MODE_INCHES) { gc_block.values.f *= MM_PER_INCH; }
        } else {
  ...
  // bit_false(value_words,bit_mask(WORD_F)); // 注：单义值词。在错误检查结束时设置。

  //[4.设置主轴转速]：S为负（完成）
  if (bit_isfalse(value_words,bit_mask(WORD_S))) { gc_block.values.s = gc_state.spindle_speed; }
  // bit_false(value_words,bit_mask(WORD_S)); // 注：单义值词。在错误检查结束时设置。

  //[5.选择工具]：不支持。只跟踪价值。T为负（完成）不是整数。大于最大刀具值。
  // bit_false(value_words,bit_mask(WORD_T)); // 注：单义值词。在错误检查结束时设置。
...
//[8.冷却液控制]：不适用
  ...
  #ifdef ENABLE_PARKING_OVERRIDE_CONTROL
    if (bit_istrue(command_words,bit_mask(MODAL_GROUP_M9))) { //已在解析器中设置为已启用。
      if (bit_istrue(value_words,bit_mask(WORD_P))) {
        if (gc_block.values.p == 0.0) { gc_block.modal.override = OVERRIDE_DISABLED; }
        bit_false(value_words,bit_mask(WORD_P));
      }
    }
  #endif
  ...
  if (gc_block.non_modal_command == NON_MODAL_DWELL) {
    if (bit_isfalse(value_words,bit_mask(WORD_P))) { FAIL(STATUS_GCODE_VALUE_WORD_MISSING); } //[P单词缺失]
    bit_false(value_words,bit_mask(WORD_P));
  }
...
//将XYZ坐标值预转换为毫米（如果适用）。
  ...
  if (gc_block.modal.units == UNITS_MODE_INCHES) {
    for (idx=0; idx<N_AXIS; idx++) { //轴索引是一致的，因此可以使用循环。
      if (bit_istrue(axis_words,bit_mask(idx)) ) {
...
//对于具有足够内存的处理器上的未来版本，所有坐标数据应存储在内存中，并仅在没有激活循环时写入EEPROM。
  ...
  if ( bit_istrue(command_words,bit_mask(MODAL_GROUP_G12)) ) { //检查是否在块中调用
    ...
    case NON_MODAL_SET_COORDINATE_DATA:
      ...
      if (gc_block.values.l != 20) {
        if (gc_block.values.l == 2) {
          if (bit_istrue(value_words,bit_mask(WORD_R))) { FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } //[G10 L2 R不受支持]
        } else { FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } //[无支持的L]
      }
      bit_false(value_words,(bit_mask(WORD_L)|bit_mask(WORD_P)));
      ...
      if (axis_command != AXIS_COMMAND_TOOL_LENGTH_OFFSET ) { //TLO阻止任何轴命令。
        if (axis_words) {
          for (idx=0; idx<N_AXIS; idx++) { //轴索引是一致的，所以可以使用循环来节省闪存空间。
            if ( bit_isfalse(axis_words,bit_mask(idx)) ) {
...
//注：圆弧跟踪需要半径和偏移，并通过错误检查预先计算。
          ...
          if (!axis_words) { FAIL(STATUS_GCODE_NO_AXIS_WORDS); } //[无轴字]
          if (!(axis_words & (bit_mask(axis_0)|bit_mask(axis_1)))) { FAIL(STATUS_GCODE_NO_AXIS_WORDS_IN_PLANE); } //[平面中无轴字]
          ...
          if (value_words & bit_mask(WORD_R)) { //弧半径模式
            bit_false(value_words,bit_mask(WORD_R));
...
//如果圆圈为逆时针方向，则反转h_x2_div_d的符号（见下图）
            ...
            */
            ...
            if (!(ijk_words & (bit_mask(axis_0)|bit_mask(axis_1)))) { FAIL(STATUS_GCODE_NO_OFFSETS_IN_PLANE); } //[平面内无偏移]
            bit_false(value_words,(bit_mask(WORD_I)|bit_mask(WORD_J)|bit_mask(WORD_K)));

            //将IJK值转换为适当的单位。
            if (gc_block.modal.units == UNITS_MODE_INCHES) {
              for (idx=0; idx<N_AXIS; idx++) { //轴索引是一致的，所以可以使用循环来节省闪存空间。
                if (ijk_words & bit_mask(idx)) { gc_block.values.ijk[idx] *= MM_PER_INCH; }
              }
            }
...
//它还允许规划器缓冲区清空并在另一个探测周期之前移出探测触发器。
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
    ...
    bit_false(value_words,(bit_mask(WORD_N)|bit_mask(WORD_F)));
  } else {
    bit_false(value_words,(bit_mask(WORD_N)|bit_mask(WORD_F)|bit_mask(WORD_S)|bit_mask(WORD_T))); //删除单义词。
  }
  if (axis_command) { bit_false(value_words,(bit_mask(WORD_X)|bit_mask(WORD_Y)|bit_mask(WORD_Z))); } //删除轴字。
...
//注意：在步骤3中已执行进给率字和轴字检查。
    if (command_words & ~(bit_mask(MODAL_GROUP_G3) | bit_mask(MODAL_GROUP_G6) | bit_mask(MODAL_GROUP_G0)) ) { FAIL(STATUS_INVALID_JOG_COMMAND) };
