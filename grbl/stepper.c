...
#define PREP_FLAG_RECALCULATE bit_mask(0)
#define PREP_FLAG_HOLD_PARTIAL_BLOCK bit_mask(1)
#define PREP_FLAG_PARKING bit_mask(2)
#define PREP_FLAG_DECEL_OVERRIDE bit_mask(3)
...
void st_generate_step_dir_invert_masks()
{
  ...
  for (idx=0; idx<N_AXIS; idx++) {
    if (bit_istrue(settings.step_invert_mask,bit_mask(idx))) { step_port_invert_mask |= get_step_pin_mask(idx); }
    if (bit_istrue(settings.dir_invert_mask,bit_mask(idx))) { dir_port_invert_mask |= get_direction_pin_mask(idx); }
  }
  #ifdef ENABLE_DUAL_AXIS
    step_port_invert_mask_dual = 0;
    dir_port_invert_mask_dual = 0;
    //注：双轴反转使用N_AXIS位设置步进和方向反转引脚。
    if (bit_istrue(settings.step_invert_mask,bit_mask(N_AXIS))) { step_port_invert_mask_dual = (1<<DUAL_STEP_BIT); }
    if (bit_istrue(settings.dir_invert_mask,bit_mask(N_AXIS))) { dir_port_invert_mask_dual = (1<<DUAL_DIRECTION_BIT); }
  #endif
}
...
void st_prep_buffer()
{
	...
	float dt_max; //最大分段时间
    float dt; //初始化段时间
    float time_var; //时间工作者变量
    float mm_var; //毫米距离工作变量
    float speed_var; //速度工作者变量
    float mm_remaining; //新线段到块末端的距离。
    float minimum_mm; //保证至少一步。
	float step_dist_remaining; //将mm_remaining转换为步
    float n_steps_remaining; // 当前剩余步数向上取整
    float last_n_steps_remaining; // 最后剩余步数向上取整
	float inv_rate;
	uint32_t cycles;
...
//注：对于加速巡航和仅巡航类型，以下计算将为0.0。
    ...
    dt_max = DT_SEGMENT; //最大分段时间
    dt = 0.0; //初始化段时间
    time_var = dt_max; //时间工作者变量
    mm_remaining = pl_block->millimeters; //新线段到块末端的距离。
    minimum_mm = mm_remaining-prep.req_mm_increment; //保证至少一步。
...
//增加分段时间，以确保分段中至少有一个步骤。覆盖并循环计算距离，直到完成最小距离或最小距离。
    ...
    */
    step_dist_remaining = prep.step_per_mm*mm_remaining; //将mm_remaining转换为步
    n_steps_remaining = ceil(step_dist_remaining); // 当前剩余步数向上取整
    last_n_steps_remaining = ceil(prep.steps_remaining); // 最后剩余步数向上取整
...
//AMASS需要执行完整的步骤。所以只需退出。
    ...
    inv_rate = dt/(last_n_steps_remaining - step_dist_remaining); //计算调整步进速率逆

    //计算预处理段每一步的CPU周期。
    cycles = ceil( (TICKS_PER_MICROSECOND*1000000*60)*inv_rate ); //（周期/步）
