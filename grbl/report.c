...
void report_realtime_status()
{
  ...
  int32_t current_position[N_AXIS]; //复制系统位置变量的当前状态
	uint8_t sp_state;
  uint8_t cl_state;
	uint8_t lim_pin_state;
  uint8_t ctrl_pin_state;
  ...
  //报告实时进给速度
    ...
    if (lim_pin_state | ctrl_pin_state | prb_pin_state) {
      ...
      if (lim_pin_state) {
            ...
            if (bit_istrue(lim_pin_state,(bit_mask(X_AXIS)|bit_mask(N_AXIS)))) { serial_write('X'); }
            if (bit_istrue(lim_pin_state,bit_mask(Y_AXIS))) { serial_write('Y'); }
          #endif
          #if (DUAL_AXIS_SELECT == Y_AXIS)
            if (bit_istrue(lim_pin_state,bit_mask(X_AXIS))) { serial_write('X'); }
            if (bit_istrue(lim_pin_state,(bit_mask(Y_AXIS)|bit_mask(N_AXIS)))) { serial_write('Y'); }
          #endif
          if (bit_istrue(lim_pin_state,bit_mask(Z_AXIS))) { serial_write('Z'); }
        #else
          if (bit_istrue(lim_pin_state,bit_mask(X_AXIS))) { serial_write('X'); }
          if (bit_istrue(lim_pin_state,bit_mask(Y_AXIS))) { serial_write('Y'); }
          if (bit_istrue(lim_pin_state,bit_mask(Z_AXIS))) { serial_write('Z'); }
