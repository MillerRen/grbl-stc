...
//归位轴搜索距离乘数。由该值乘以循环行程计算得出。
  ...
  #define DUAL_AXIS_CHECK_ENABLE      bit_mask(0)
  #define DUAL_AXIS_CHECK_TRIGGER_1   bit_mask(1)
  #define DUAL_AXIS_CHECK_TRIGGER_2   bit_mask(2)
#endif
...
//在限制引脚改变时，启用看门狗定时器以创建短延迟。
  //void LIMIT_INT_vect() { if (!(WDTCSR & (1<<WDIE))) { WDTCSR |= (1<<WDIE); } }
  void WDT_vect() //看门狗定时器
  {
...
void limits_go_home(uint8_t cycle_mask)
{
  ...
  for (idx=0; idx<N_AXIS; idx++) {
    ...
    if (bit_istrue(cycle_mask,bit_mask(idx))) {
  ...
  limit_state, axislock, n_active_axis;
  do {
    ...
    for (idx=0; idx<N_AXIS; idx++) {
      ...
      if (bit_istrue(cycle_mask,bit_mask(idx))) {
        ...
        if (bit_istrue(settings.homing_dir_mask,bit_mask(idx))) {
